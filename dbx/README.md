...
```bash
conda create -n workflows python=3.9
...
```
This command will create your personal repository under `/Repos/<username>/workflows`.
...
- To trigger the release pipeline, get the current version from the `workflows/__init__.py` file and tag the current code version:
```
